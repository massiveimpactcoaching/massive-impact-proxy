...
app.post('/notify', async (req, res) => {
  try {
    const { name, business, email, phone, package: pkg, total, callTime, notes } = req.body;
    ...
    const html = `
      <div style="font-family: DM Sans, Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #0D1B2A;">
        <div style="background: #0D1B2A; padding: 28px 32px; border-radius: 12px 12px 0 0;">
          ...
          <p style="color: rgba(255,255,255,0.6); margin: 6px 0 0; font-size: 14px;">Phase 2 conversation completed on /lawn</p>
        </div>
        <div style="background: #f7faff; padding: 32px; border: 1px solid #dde4ee; border-top: none; border-radius: 0 0 12px 12px;">
          ...
          <h2 style="font-size: 16px; color: #0D1B2A; margin: 0 0 16px;">Send Them These Onboarding Questions</h2>
          ...
          ${notes ? `<div style="margin-top:24px;"><h2 style="font-size:16px;color:#0D1B2A;margin:0 0 12px;">Additional Notes</h2><p style="color:#5A7090;font-size:14px;">${notes}</p></div>` : ''}
    ...
    await transporter.sendMail({
      from: `"Massive Impact Bot" <${process.env.GMAIL_USER}>`,
