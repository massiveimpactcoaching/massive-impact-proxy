...
app.post('/quote', async (req, res) => {
  try {
    ...
    const COMPLETE_PACKAGE_ITEMS = [
      { name: 'Core Bot — 24/7 lead capture, instant quotes, full brand customization', setup: '$500', monthly: '$200/mo' },
      { name: 'Inline Scheduling — customers book directly in the chat', setup: '$100', monthly: '$25/mo' },
      { name: 'Follow-up Automation — automated reminders and re-engagement', setup: '$150', monthly: '$50/mo' },
      { name: 'Review Management — automatic Google review requests after every job', setup: '$75', monthly: '$30/mo' },
      { name: 'Site Navigator Bot — homepage guide that routes every visitor', setup: '$100', monthly: '$40/mo' },
    ];

    const isCompletePackage = (pkg || '').toLowerCase().includes('complete package');

    let rows = '';
    if (isCompletePackage) {
      rows = COMPLETE_PACKAGE_ITEMS.map(item => `
        <tr>
          <td style="padding:12px 16px;border-bottom:1px solid #eee;color:#0D1B2A;font-size:14px;width:60%;">${item.name}</td>
          <td style="padding:12px 16px;border-bottom:1px solid #eee;color:#5A7090;font-size:13px;text-align:right;white-space:nowrap;">${item.setup}</td>
          <td style="padding:12px 16px;border-bottom:1px solid #eee;color:#1A6BCC;font-size:13px;font-weight:600;text-align:right;white-space:nowrap;">${item.monthly}</td>
        </tr>`).join('');
      rows += `<tr style="background:#f7faff;">
        <td style="padding:12px 16px;color:#0D1B2A;font-size:14px;font-weight:700;">Complete Package Total</td>
        <td style="padding:12px 16px;color:#0D1B2A;font-size:14px;font-weight:700;text-align:right;">$800 setup</td>
        <td style="padding:12px 16px;color:#1A6BCC;font-size:14px;font-weight:700;text-align:right;">$345/mo</td>
      </tr>
      <tr>
        <td colspan="3" style="padding:8px 16px;font-size:12px;color:#5A7090;font-style:italic;">Bundled pricing saves you $125 on setup vs. purchasing individually</td>
      </tr>`;
    } else {
      ...
      rows = lines.map(line => {
        ...
        return `<tr>
          <td colspan="3" style="padding:12px 16px;border-bottom:1px solid #eee;color:#0D1B2A;font-size:14px;">${clean}</td>
        </tr>`;
      }).join('');
    }
    ...
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <td style="background:#fff;padding:40px;border-left:1px solid #dde4ee;border-right:1px solid #dde4ee;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee;border-radius:8px;overflow:hidden;margin-bottom:24px;">
            <tr style="background:#f7faff;">
              <td style="padding:10px 16px;font-size:12px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#5A7090;width:60%;">Feature</td>
              <td style="padding:10px 16px;font-size:12px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#5A7090;text-align:right;">Setup</td>
              <td style="padding:10px 16px;font-size:12px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#5A7090;text-align:right;">Monthly</td>
            </tr>
            ${rows || '<tr><td colspan="3" style="padding:12px 16px;color:#5A7090;font-size:14px;">Custom package</td></tr>'}
          </table>
      ...
      <tr>
        <td style="background:#0D1B2A;padding:28px 40px;border-radius:0 0 12px 12px;text-align:center;">
          <p style="color:#fff;font-size:15px;font-weight:700;margin:0 0 4px;">Max</p>
          <p style="color:rgba(255,255,255,0.55);font-size:13px;margin:0 0 4px;">AI Assistant · Massive Impact Custom Assistants</p>
          <p style="color:rgba(255,255,255,0.4);font-size:13px;margin:0 0 16px;">massiveimpactcoaching.com · <a href="mailto:chris@massiveimpactcoaching.com" style="color:#3B9EFF;text-decoration:none;">Reply to Chris</a></p>
